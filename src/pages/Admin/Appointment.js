...
const Appointment = () => {
    ...
    debounce(async (date) => {
      try {
        ...
        const res = await axios.get(
          `http://localhost:8005/api/appointment/Datewiseappointment/${formattedDate}`
        );
        if (res.status === 200) {
  ...
  const fetchMonthlyAppointments = useCallback(async (monthYear) => {
    try {
      console.log(`Fetching data for: ${monthYear}`); // Debugging

      const res = await axios.get(
        `http://localhost:8005/api/appointment/Monthlyappointment/${monthYear}`
      );
      if (res.status === 200) {
  ...
  useEffect(() => {
    fetchDailyAppointments(new Date());
    fetchMonthlyAppointments(activeMonth); // Use the active month
  }, [fetchDailyAppointments, fetchMonthlyAppointments, activeMonth]);

  // ✅ Fetch daily data when selectedDate changes
  useEffect(() => {
    fetchDailyAppointments(selectedDate);
  }, [selectedDate, fetchDailyAppointments]);
  ...
  const handleMonthChange = ({ activeStartDate }) => {
    ...
    if (newMonthYear !== activeMonth) {
      ...
      fetchMonthlyAppointments(newMonthYear);
    }
  };
